<!DOCTYPE html>
<html lang="en">
<head>
    <meta charset="UTF-8">
    <meta name="viewport" content="width=device-width, initial-scale=1.0">
    <title>Tạo khách hàng</title>
</head>
<body>
    <h1>abcd</h1>
    <form action="{{ route('admin.customer.update', $customer->id) }}" method="POST" enctype="multipart/form-data">
        @csrf
        
        <label for="registration_id">ID:</label>
        <input type="text" id="registration_id" name="registration_id" value="{{ old('registration_id', $customer->registration_id) }}"  required maxlength="8">
        @error('registration_id')
            <p style="color: red;">{{ $message }}</p>
        @enderror
        <div>
            <label for="name">Tên khách hàng:</label>
            <input type="text" name="name" id="name" value="{{ old('name', $customer->name) }}">
            @error('name')
                <p style="color: red;">{{ $message }}</p>
            @enderror
        </div>
        <div>
            <label for="email">Email:</label>
            <input type="email" name="email" value="{{ old('email', $customer->email) }}" required>
            @error('email')
                <p style="color: red;">{{ $message }}</p>
            @enderror
        </div>

        <div>
            <label>Giới tính:</label>
            <label><input type="radio" name="gender" value="male" {{ $customer->gender == 'male' ? 'checked' : '' }}> Nam</label>
            <label><input type="radio" name="gender" value="female" {{ $customer->gender == 'female' ? 'checked' : '' }}> Nữ</label>
            @error('gender')
                <p style="color: red;">{{ $message }}</p>
            @enderror
        </div>

        <div>
            <label>Sở thích:</label>
            @foreach ($hobbies as $hobby)
                <label>
                    <input type="checkbox" name="hobbies[]" value="{{ $hobby }}" {{ in_array($hobby, $selectedHobbies) ? 'checked' : '' }}> {{ $hobby }}
                </label>
            @endforeach
            @error('hobbies')
                <p style="color: red;">{{ $message }}</p>
            @enderror
        </div>

        <div>
            <label for="country">Quốc gia:</label>
            <select name="country" id="country">
                @foreach ($countries as $country)
                    <option value="{{ $country }}" {{ $customer->country == $country ? 'selected' : '' }}>{{ $country }}</option>
                @endforeach
            </select>
            @error('country')
                <p style="color: red;">{{ $message }}</p>
            @enderror
        </div>
        <label for="password">Mật khẩu (để trống nếu không thay đổi):</label>
        <input type="password" name="password">
        @error('password')
            <p style="color: red;">{{ $message }}</p>
        @enderror

        <div>
            <label for="profile_picture">Ảnh đại diện:</label>
            @if ($customer->profile_picture)
                <img src="{{ asset('storage/' . $customer->profile_picture) }}" alt="Profile Picture" width="100">
            @endif
            <input type="file" name="profile_picture" id="profile_picture">
            @error('profile_picture')
                <p style="color: red;">{{ $message }}</p>
            @enderror
        </div>

        <button type="submit">Cập nhật</button>
    </form>
</body>
</html>
